                ...
                <el-card class="box-card" style="    width: 100%;">
                    ...
                    <el-form ref="tokenFormRef" :model="tokenForm" :rules="tokenRules" label-width="auto"
                        ...
                        <el-form-item label="标识" prop="identifier">
                            <el-input v-model="tokenForm.identifier" autocomplete="on" />
                        </el-form-item>
                        <el-form-item label="网址" prop="server">
                            <el-input v-model="tokenForm.server" autocomplete="on" />
                        </el-form-item>
                        <el-form-item class="center-button">
                            <el-button size="large" :loading="loading" type="primary"
                                @click="submitTokenForm(tokenFormRef)">登录</el-button>
                            <el-button size="large" @click="resetForm(tokenFormRef)">重置</el-button>
                        </el-form-item>
                    </el-form>
                    <template #footer>
                        <div class="card-header">
...
const submitForm = async (formEl: FormInstance | null) => {
    if (!formEl) return;
    await formEl.validate(async (isValid: boolean, invalidFields?: ValidateFieldsError) => {
        if (isValid) {
            console.log('submit!');
            loading.value = true;
            try {
